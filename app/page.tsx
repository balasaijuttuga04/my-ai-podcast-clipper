...
import type { Highlight, RenderedClip, TranscriptSegment, WordTimestamp } from "@/lib/types";
...
export default function HomePage() {
  ...
  async function generateClips() {
    ...
    try {
      ...
      if (!transcriptionRes.ok) throw new Error((await transcriptionRes.json()).error || "Transcription failed.");
      ...
      if (!transcription.text || !transcription.words?.length) {
        throw new Error("No speech was detected. Try a clearer audio/video file.");
      }
      ...
      if (!highlightsRes.ok) throw new Error((await highlightsRes.json()).error || "Highlight detection failed.");
      const { highlights } = (await highlightsRes.json()) as { highlights: Highlight[] };

      if (!highlights.length) {
        throw new Error("The AI could not find strong highlights. Try a longer or more conversational episode.");
      }
      ...
      <div className="mx-auto max-w-6xl">
        <section className="mb-8 rounded-[2rem] border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-2xl">
          <div className="flex items-center gap-3 text-cyan-300">
            ...
            <span className="text-sm font-semibold uppercase tracking-[0.3em]">AI Podcast Clipper</span>
          </div>
          <h1 className="mt-4 max-w-3xl text-4xl font-bold leading-tight md:text-6xl">
            Turn long podcasts into captioned vertical shorts.
          </h1>
          <p className="mt-4 max-w-2xl text-slate-300">
            Upload a podcast file, transcribe with Groq Whisper, detect strong moments with an LLM,
            and render 9:16 clips with burned-in captions using FFmpeg.
          </p>
        </section>
            ...
            <div className="rounded-3xl bg-slate-900/70 p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="text-sm text-slate-300">Preferred clip length</span>
                  <select
                    value={clipLength}
                ...
                <label className="block">
  <span className="text-sm text-slate-300">Smart crop focus</span>
  <select
    value={cropMode}
    onChange={(e) =>
      setCropMode(e.target.value as "center" | "left" | "right")
    }
    className="mt-2 w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3"
  >
      ...
      <option key={n} value={n}>
        {n} clip{n > 1 ? "s" : ""}
              ...
              <button
                ...
                disabled={!file || step === "transcribing" || step === "analyzing" || step === "rendering"}
              ...
              {error && <p className="mt-4 rounded-2xl border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-200">{error}</p>}
            </div>
          </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-3xl bg-slate-900/70 p-5 text-sm text-slate-300">
              ...
              <p className="mt-2">
                Word-level timestamps attach start/end seconds to each word. The renderer groups
                those words into short subtitle lines, writes an ASS subtitle file, and FFmpeg burns
                it into the final 1080x1920 MP4.
              </p>
            </div>
          </aside>
        </div>
